...
func _ready():
	...
	var score_script = get_parent().get_node("top_ui")
	score_script.connect("times_up", _on_times_up)
	score_script.connect("won", _on_won)
	...
	for i in width:
		for j in height:
			if all_pieces[i][j] != null and match_at(i, j, all_pieces[i][j].color):
				find_matches()
				current_streak += 1
				top_ui.setStreak(current_streak)
